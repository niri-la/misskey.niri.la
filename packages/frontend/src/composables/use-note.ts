...
export function calculateMuteStatus<
	...
	ReturnTypeA = CheckOnly extends true ? boolean : Array<string | string[]> | false | 'sensitiveChannel' | 'sensitiveMute',
	ReturnTypeB = CheckForSensitiveMedia extends true ? ReturnTypeA : Exclude<ReturnTypeA, 'sensitiveChannel' | 'sensitiveMute'>,
>(
	...
	collapseSensitiveChannel: CheckForSensitiveMedia = false as never,
): ReturnTypeB {
	if (mutedWords != null) {
	...
	if (collapseSensitiveChannel && noteToCheck.channel?.isSensitive) return 'sensitiveChannel' as ReturnTypeB;
...
export function useNote(
	...
	// ミュート判定
	const muted = ref($i ? calculateMuteStatus(appearNote, $i, $i.mutedWords, inTimeline && !tl_withSensitive.value, false, collapseSensitiveChannel) : false);
